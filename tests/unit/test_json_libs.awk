...
function test_json_valid_object(   r, _out_type) {
  if (!LIBS_LOADED["json"]) { TESTS_SKIPPED++; return }
  r = json_decode("{\"a\":1}", _out, _out_type)
  assert_eq(r, 1, "json_decode: valid object")
}

function test_json_decode_invalid(   r, _out_type) {
  if (!LIBS_LOADED["json"]) { TESTS_SKIPPED++; return }
  r = json_decode("{invalid}", _out, _out_type)
